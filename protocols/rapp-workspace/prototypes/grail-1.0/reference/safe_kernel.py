"""Minimal first-Grail kernel. Effects and effective grants never come from the learned graph."""
...
class Controller:
    ...
    def guard(self, subject, action, inherited=None):
        require(action not in DISABLED, "disabled-first-grail:" + action)
    ...
    def seed(self, subject):
        ...
        with self.transaction():
            ref = self._emit(self._payload("seed", subject, generation="first-grail",
    ...
    def capture_octets(self, subject, octets, *, inherited=(), consistency="supplied-immutable-octets"):
        ...
        with self.transaction():
            ...
            deployment = self._receipt("safe_deployment", source, "refused", "external-effects",
                "unproved-deployment-disabled", {"reason": "first-Grail-external-effects-disabled"}, subject, restrictions)
