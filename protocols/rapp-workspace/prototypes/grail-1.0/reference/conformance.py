"""Blocking first-Grail safety-kernel conformance; no experimental grants or effects."""
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path(".validation/first-grail-safe-conformance"))
    ...
    require(initial == encode(manifest()) and check_index(), "first-Grail validator/runtime pin drift")
    ...
    require(result.wasSuccessful() and result.testsRun > 0 and not result.skipped,
            "blocking first-Grail vectors failed")
    ...
    report = {
        "spec_id": "rapp-workspace/grail-1.0", "brand": "RAPP Workspace/1 Grail",
        ...
        "scanner_scope": "RAPP-integrity-only", "signed_grail_activation": False,
        "safely_deployable": False, "external_effects": "disabled", "experimental_results_are_not_acceptance": True,
    }
